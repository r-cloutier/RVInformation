...
header = '\clearpage\n\\begin{turnpage}\n\\begin{deluxetable}{cccccccccccccccccc}\n\\tabletypesize{\scriptsize}\n\\tablecaption{Stellar parameters from the \\citetalias{sullivan15} synthetic catalog \label{table:stars}}\n\\tablewidth{0pt}\n\\tablehead{TOI & $\\alpha$ & $\\delta$ & $P$ & $m_p$ & $K$ & $S$ & $M_s$ & $T_{\\text{eff}}$ & Distance & $B$ & $V$ & $R$ & $Y$ & $J$ & $H$ & $\\text{med}(v\\sin{i_s})$ & \\\ & $[$deg$]$ & $[$deg$]$ & $[$days$]$ & $[\\text{M}_{\oplus}]$ & $[$m s$^{-1}]$ & $[\\text{S}_{\oplus}]$ & $[\\text{M}_{\odot}]$ & $[$K$]$ & $[$pc$]$ & & & & & & & $[$km s$^{-1}]$}\n'
...
def create_startable(self):
    ...
    for i in range(self.nstars):
        ...
        theta = prefix, i, self.ras_med[i], self.decs_med[i], self.Ps_med[i], \
                ...
                self.Bmags_med[i], self.Vmags_med[i], self.Rmags_med[i], self.Ymags_med[i], \
                self.Jmags_med[i], self.Hmags_med[i], self.vsinis_med[i], suffix
        body += '%s%.4d & %.2f & %.2f & %.3f & %.2f & %.2f & %.1f & %.2f & %i & %.1f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f & %.2f \\\\ %s'%theta
